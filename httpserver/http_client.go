...
func NewTHttpClient(urlstr string) (thrift.TTransport, error) {
...
func NewTHttpPostClient(urlstr string) (thrift.TTransport, error) {
	parsedURL, err := url.Parse(urlstr)
	if err != nil {
		return nil, err
	}
...
func (p *THttpClient) Flush(ctx context.Context) error {
	...
	if response.StatusCode != http.StatusOK {
		...
		return thrift.NewTTransportException(thrift.UNKNOWN_TRANSPORT_EXCEPTION, "HTTP Response code: "+strconv.Itoa(response.StatusCode))
	}
	p.response = response
	return nil
}
